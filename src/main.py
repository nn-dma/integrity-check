...
def ensure_id(features, allowlist):
    ...
    for ii in features:
        ...
        if len(tag_on_feature) < 1:
            raise Exception(
                f"Every feature needs unique ID: {ii} does not contain ID"
            )
...
def ensure_urs_scenarios(features):
    ...
    for feature in features:
        if "URS" in feature.tags:
            assert (
                len(feature.scenarios) > 0
            ), "URS features must have test cases"
            tagged_pv = [
                scenario.tags
                for scenario in feature.scenarios
                if "PV" in scenario.tags
            ]
            assert (
                len(tagged_pv) > 0
...
def ensure_step_implementations(features):
    """
    This is probably not feasible, as we need to actually
    load the step implementation,
    which means we would need to inherit transitive
    dependencies of the consuming project.
    """
    pass
...
def check_uniqueness_of_requirements(
    ...
    duplicates = [
        tag for tag, count in tag_frequency_dict.items() if count > 1
    ]
...
def get_tags_of_markdown_files(docs_path):
    ...
    for file in markdown_files:
        ...
        text = f.read()
        tags = [
            ii.replace("  - ", "") for ii in re.findall(r"  - \w\S*", text)
        ]
        taglist.extend(tags)
...
if __name__ == "__main__":
    ...
    msg = f"""
    The following URS IDs were found in the .feature files: {feature_tags}
    """
    # print(msg)
    ...
    print(
        f"Are all URS tags unique? {is_unique}"
    )  # This will print False if there are duplicates
    ...
    msg = f"""
    The following URS IDs are referenced in the documentation: {docs_tags}
    """
    # print(msg)

    # Verify that all urs_ids have references to design - i.e.
    # all urs_ids need to be in design tags
    ...
    if not in_ds:
        ...
        print(
            """
            Traceability check successful.
            All URS IDs are unique and referenced in the documentation.
            """
        )
